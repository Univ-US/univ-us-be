...
import com.univus.app.lms.mapper.LmsProfEnrolleeMapper;
...
public class LmsProfEnrolleeServiceImpl implements LmsProfEnrolleeService {
    ...
    private final LmsProfEnrolleeMapper lmsProfEnrolleeMapper;
    private final LmsEnrolleeExcelExporter enrolleeExcelExporter;

    /** 학기 드롭다운 (코드만 반환 — 라벨은 FE가 /api/common-codes/SEM_TERM 로 매핑) */
    @Override
    @Transactional(readOnly = true)
    public List<LmsProfSemesterDto> getSemesters() {
        return lmsProfEnrolleeMapper.selectSemesters();
    }

    /** 교수 담당 강의 드롭다운 (학기 선택 optional) */
    @Override
    @Transactional(readOnly = true)
    public List<LmsProfLectureDto> getLectures(Long professorMemberId, Long semId) {
        Long lmsPrfId = lmsProfEnrolleeMapper.findLmsPrfIdByMemberId(professorMemberId);
        if (lmsPrfId == null) {
            return List.of();
        }
        return lmsProfEnrolleeMapper.selectLecturesByProfessor(lmsPrfId, semId);
    }

    /** PLM-003 : 강의별 수강생 목록(현재 페이지) + 통계 카드 3개 (검색·제출필터·정렬·페이지네이션) */
    @Override
    @Transactional(readOnly = true)
    public LmsProfEnrolleeDto.ListResDto getLectureStudents(Long professorMemberId, Long lecId,
                                                            String search, String submission,
                                                            String sort, String order,
                                                            int page, int size) {
        LmsProfEnrolleeDto.ListResDto data =
                buildFilteredStudents(professorMemberId, lecId, search, submission, sort, order);

        // 필터·정렬된 전체 목록을 페이지로 슬라이스 (통계 카드는 전체 기준 유지)
        List<LmsProfEnrolleeDto.RowResDto> all = data.getStudents();
        ...
        data.setPagination(LmsProfEnrolleeDto.PageInfo.builder()
                .page(safePage)
                .size(safeSize)
    ...
    private LmsProfEnrolleeDto.ListResDto buildFilteredStudents(Long professorMemberId, Long lecId,
                                                               String search, String submission,
                                                               String sort, String order) {
        Long professorLmsPrfId = requireProfessorLmsPrfId(professorMemberId);

        LmsProfLectureDto lecture = lmsProfEnrolleeMapper.selectLectureHeader(lecId, professorLmsPrfId);
        if (lecture == null) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "본인이 담당하는 강의가 아닙니다.");
        }

        int totalAssignments = lmsProfEnrolleeMapper.countAssignments(lecId);
        List<LmsProfEnrolleeDto.Row> rows =
                lmsProfEnrolleeMapper.selectStudentRows(lecId, professorLmsPrfId, blankToNull(search));

        // 매핑 row → 응답 RowResDto (출석률 계산·총 과제 수 세팅)
        List<LmsProfEnrolleeDto.RowResDto> students = new ArrayList<>(rows.size());
        for (LmsProfEnrolleeDto.Row r : rows) {
            students.add(LmsProfEnrolleeDto.RowResDto.builder()
                    .enrollmentId(r.getEnrollmentId())
                    .memberId(r.getMemberId())
                    .studentName(r.getStudentName())
                    .studentNo(r.getStudentNo())
                    .imageUrl(r.getImageUrl())
                    .attendanceRate(percent(r.getPresentCount(), r.getAttendanceTotal()))
                    .submittedCount(r.getSubmittedCount())
        ...
        LmsProfEnrolleeDto.SummaryResDto summary = buildSummary(students, totalAssignments);
        // 목록에 제출 필터 + 정렬 적용 (서버사이드)
        List<LmsProfEnrolleeDto.RowResDto> view =
                applyFilterAndSort(students, blankToNull(submission), blankToNull(sort), order);

        return LmsProfEnrolleeDto.ListResDto.builder()
                .lecture(lecture)
                .summary(summary)
                .students(view)
                .build();
    ...
    @Override
    @Transactional(readOnly = true)
    public LmsProfEnrolleeDto.ReportResDto getStudentReport(Long professorMemberId, Long lecId, Long studentMemberId) {
        Long professorLmsPrfId = requireProfessorLmsPrfId(professorMemberId);

        LmsProfEnrolleeDto.ReportRow row =
                lmsProfEnrolleeMapper.selectStudentReportHeader(lecId, professorLmsPrfId, studentMemberId);
        if (row == null) {
        ...
        // 과제별 점수 (매핑 row → 응답: 제출/채점 여부 계산 보강)
        List<LmsProfEnrolleeDto.AssignmentScoreRow> scoreRows =
                lmsProfEnrolleeMapper.selectAssignmentScores(lecId, row.getStudentLmsPrfId());
        ...
        return LmsProfEnrolleeDto.ReportResDto.builder()
                .memberId(row.getMemberId())
                .studentName(row.getStudentName())
                .studentNo(row.getStudentNo())
                .imageUrl(row.getImageUrl())
                .lectureName(row.getLectureName())
                .attendanceRate(percent(row.getAttendancePresent(), row.getAttendanceTotal()))
                .submittedCount(row.getSubmittedCount())
                .totalAssignments(totalAssignments)
                .averageScore(row.getAverageScore())
                .attendancePresent(row.getAttendancePresent())
                .attendanceLate(row.getAttendanceLate())
                .attendanceAbsent(row.getAttendanceAbsent())
                .attendanceTotal(row.getAttendanceTotal())
    ...
    private Long requireProfessorLmsPrfId(Long memberId) {
        Long lmsPrfId = lmsProfEnrolleeMapper.findLmsPrfIdByMemberId(memberId);
        if (lmsPrfId == null) {
    ...
    private LmsProfEnrolleeDto.SummaryResDto buildSummary(List<LmsProfEnrolleeDto.RowResDto> students, int totalAssignments) {
        int total = students.size();
        int averageAttendanceRate = total == 0 ? 0
                : (int) Math.round(students.stream()
                        .mapToInt(LmsProfEnrolleeDto.RowResDto::getAttendanceRate)
                        .average()
                        .orElse(0));
        ...
        return LmsProfEnrolleeDto.SummaryResDto.builder()
                .totalStudents(total)
    ...
    private List<LmsProfEnrolleeDto.RowResDto> applyFilterAndSort(List<LmsProfEnrolleeDto.RowResDto> students,
                                                                 String submission, String sort, String order) {
        List<LmsProfEnrolleeDto.RowResDto> view = students;
        if (submission != null) {
            view = view.stream().filter(s -> matchesSubmission(s, submission)).toList();
        }
        Comparator<LmsProfEnrolleeDto.RowResDto> comparator = resolveComparator(sort, order);
        if (comparator != null) {
    ...
    private boolean matchesSubmission(LmsProfEnrolleeDto.RowResDto student, String submission) {
    ...
    private Comparator<LmsProfEnrolleeDto.RowResDto> resolveComparator(String sort, String order) {
        if (sort == null) {
        ...
        if ("name".equalsIgnoreCase(sort)) {
            ...
            return Comparator.comparing(LmsProfEnrolleeDto.RowResDto::getStudentName, Comparator.nullsLast(direction));
        }
        if ("studentNo".equalsIgnoreCase(sort)) {
            Comparator<String> direction = desc ? Comparator.reverseOrder() : Comparator.naturalOrder();
            return Comparator.comparing(LmsProfEnrolleeDto.RowResDto::getStudentNo, Comparator.nullsLast(direction));
        }
        if ("attendance".equalsIgnoreCase(sort)) {
            Comparator<LmsProfEnrolleeDto.RowResDto> base = Comparator.comparingInt(LmsProfEnrolleeDto.RowResDto::getAttendanceRate);
        ...
        if ("score".equalsIgnoreCase(sort)) {
            ...
            return Comparator.comparing(LmsProfEnrolleeDto.RowResDto::getAverageScore, Comparator.nullsLast(direction));
        }
