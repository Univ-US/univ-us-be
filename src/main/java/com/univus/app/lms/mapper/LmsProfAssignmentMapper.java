...
@Mapper
public interface LmsProfAssignmentMapper {

    /** 교수 본인 LMS_PRF_ID 조회 (없으면 null) */
    Long findLmsPrfIdByMemberId(@Param("memberId") Long memberId);

    /** 등록 폼 과목(강의) 드롭다운 — 교수 담당 강의 전체 */
    List<LmsProfAssignmentDto.LectureRow> selectLecturesByProfessor(@Param("professorLmsPrfId") Long professorLmsPrfId);
    ...
    List<LmsProfAssignmentDto.AssignmentRow> selectAssignmentsByCoursePaged(@Param("professorLmsPrfId") Long professorLmsPrfId,
                                                                     @Param("lecId") Long lecId,
                                                                     @Param("offset") int offset,
                                                                     @Param("size") int size);

    /** 과제 단건 + 집계 (등록/수정 응답용 — 본체만) */
    LmsProfAssignmentDto.AssignmentRow selectAssignmentById(@Param("assignmentId") Long assignmentId);

    /** 주어진 과제들(현재 페이지 assignmentId들)의 유효(ACT) 첨부 목록 (서비스에서 과제별 그룹핑) */
    List<LmsProfAssignmentDto.AttachmentRow> selectActiveAttachmentsByAssignmentIds(
            @Param("assignmentIds") List<Long> assignmentIds);

    /** 과제 1건의 유효(ACT) 첨부 목록 */
    List<LmsProfAssignmentDto.AttachmentRow> selectActiveAttachmentsByAssignmentId(
            @Param("assignmentId") Long assignmentId);

    /** 과제 본체 INSERT — assignmentId는 selectKey(SEQ_LECTURE_ASSIGNMENT)로 채번되어 param에 채워짐 */
    int insertAssignment(LmsProfAssignmentDto.InsertParam param);
    ...
    List<LmsProfAssignmentDto.AttachmentDiskRow> selectAttachmentsByAssignmentId(@Param("assignmentId") Long assignmentId);
